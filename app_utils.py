...
# Source audio key of a clip, without the model suffix.
# e.g. "Chakkani_Raja_seg0058-baseline_20s" -> "Chakkani_Raja_seg0058"
def get_source_key(source_clip_id):
    return re.sub(r"-(baseline|finetuned)(_\d+s)?$", "", str(source_clip_id))


# Shuffle likert trials so that the baseline and fine-tuned versions of the same
# source clip are never played one after the other. Returns a shuffled dataframe.
def shuffle_likert_trials(df, seed=None, max_attempts=1000):
    if isinstance(seed, str):
        seed = seed_from_string(seed)

    rng = random.Random(seed)
    source_keys = df["source_clip_id"].map(get_source_key).tolist()
    order = list(range(len(df)))

    for _ in range(max_attempts):
        rng.shuffle(order)
        has_adjacent_repeat = any(
                                    source_keys[a] == source_keys[b]
...
def render_instructions_expander(page_name, likert_count=12, pairwise_count=6):
    if page_name == "welcome":
        ...
        with st.expander(title, expanded=True):
            st.markdown(
                            ...
                            - :blue[Rating AI generated clips]: Rate {likert_count} AI generated carnatic musical clips
                            - :blue[Clip A v/s Clip B]: Compare {pairwise_count} pairs of AI generated music ({pairwise_count * 2} clips) from two different music generation models.
                            
                            ****:blue[Approximate time to complete the survey:]**** 20 ~ 25 mins
        ...
        with st.expander(title, expanded=True):
            st.markdown(
                        f"""
                        ### Task
                        You will rate **{likert_count} audio clips**.
        ...
        with st.expander(title, expanded=True):
            st.markdown(
                            f"""
                            ### Task
                            You will evaluate **{pairwise_count} comparison pairs** of audio clips.
